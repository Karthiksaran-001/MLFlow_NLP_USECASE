...
def featurize(config_path, params_path):
    ## read config files
    ...
    featured_data_dir_path = os.path.join(artificats["ARTIFACTS_DIR"] , artificats["FEATURIZED_DATA"])
    create_directories([featured_data_dir_path])
    featurized_train_data_path = os.path.join(featured_data_dir_path , artificats["FEATURIZED_OUT_TRAIN"])
    featurized_test_data_path =  os.path.join(featured_data_dir_path , artificats["FEATURIZED_OUT_TEST"])
...
if __name__ == '__main__':
    ...
    try:
        ...
        featurize(config_path=parsed_args.config, params_path=parsed_args.params)
